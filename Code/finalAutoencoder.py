...
def processing_map(input_images, output_images):
    input_data = tf.expand_dims(tf.clip_by_value(tf.divide(tf.add(input_images, 60000), 210000.0), 0., 1.), axis=-1)
    output_data = tf.expand_dims(tf.clip_by_value(tf.divide(tf.add(output_images, 60000), 210000.0), 0., 1.), axis=-1)
...
autoencoder.save('/home/pmmf/MastersCode/autoencoderModelsv2/MultipurposeAutoencoder')